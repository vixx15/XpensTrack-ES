...
public class TransactionsController(IMediator mediator) : ControllerBase
{
    ...
    public async Task<IActionResult> UpdateDescription(Guid id, [FromBody] UpdateTransactionRequest request)
    {
        ...
        UpdateTransactionTransferWallet? transactionTransferTarget = null;
        if (request.TransferDetails is not null)
        {
            transactionTransferTarget = new UpdateTransactionTransferWallet(
                ToWalletId: request.TransferDetails.ToWalletId,
    ...
    {
        var userId = User.FindFirstValue(ClaimTypes.NameIdentifier);
        var result = await mediator.Send(request: new GetTransactionById(Id: id, UserId: userId!));
