...
public class RevertTransactionOnWalletHandler(IDocumentStore documentStore)
    : IRequestHandler<RevertTransactionOnWallet>
{
    public async Task Handle(RevertTransactionOnWallet command, CancellationToken ct)
    {
        ...
        stream.AppendMany(stream.Aggregate!.RevertTransaction(
            command.UserId, command.TransactionId,
            command.Amount, command.DefaultCurrencyAmount, command.OriginalDirection, command.OccurredAt));
        ...
        if (command.Transfer is { } transfer)
        {
            ...
            toStream.AppendMany(toStream.Aggregate!.RevertTransaction(
                command.UserId, command.TransactionId,
